...
- startsampleapp.py: the main program to start a webapp (WeApRous) process
...
webpage at [http://IP:8080](http://IP:8080) and is redirected to be served at backend [http://IP:9000](http://IP:9000) address
...
```
In Figure 3, a user webapp is started at port [http://IP:8000](http://IP:8000) with two handler at method [PUT] path
